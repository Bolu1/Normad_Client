...
const EditJob = () => {
    ...
    if (Cookies.get("profile") == undefined) {
      ...
      const get = async () => {
        const res = await fetch(
          `https://normad-server.onrender.comgetJobDetails?post=${id}`,
          {
            method: "POST",
            headers: {
  ...
  const onSubmit = (e) => {
    ...
    if (data.title.length > 7) {
      ...
      const get = async () => {
        const res = await fetch(
          `https://normad-server.onrender.comeditJob?post=${id}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      <div class="container">
        ...
        <form onSubmit={onSubmit}>
          ...
          <div class="form-group mt-5 mb-5">
            ...
            <textarea
              class="form-control xl"
              placeholder="seprate with commas"
              id="exampleFormControlTextarea1"
              rows="3"
              value={data.skills}
              onChange={(e) =>
          ...
          <button type="submit" class="btn btn-dark px-4 btn-md">
            Edit
          </button>
        </form>
        <br />
